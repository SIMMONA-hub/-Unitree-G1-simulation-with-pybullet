...
import config
from motor_indices import G1_29_JointIndex
...
MOTOR_SENSOR_NUM = 3
NUM_MOTOR_IDL_GO = 20
NUM_MOTOR_IDL_HG = 29  # Number of body motors in G1
...
class UnitreeSdk2Bridge:
    ...
    def __init__(self, mj_model, mj_data):
        ...
        # Separate the motors into body and hand motors for G1
        if config.ROBOT == "g1":
            self.body_motors = NUM_MOTOR_IDL_HG
            self.hand_motors = self.num_motor - self.body_motors
            print(f"Body motors: {self.body_motors}, Hand motors: {self.hand_motors}")
        else:
            self.body_motors = self.num_motor
            self.hand_motors = 0
        ...
        self.lowStateThread = RecurrentThread(
            interval=1.0/config.ARM_CONTROL_FREQ,  # 250Hz
        ...
        # Initialize hand state and command for G1
        if config.ROBOT == "g1" and self.hand_motors > 0:
            # Initialize left hand state
            self.left_hand_state = HandState_default()
            self.left_hand_state.motor_state = [
                MotorState_(
                    mode=0,  # Default mode
                    q=0.0,   # Initial position
                    dq=0.0,  # Initial velocity
                    ddq=0.0, # Initial acceleration
                    tau_est=0.0,  # Initial torque estimate
                    temperature=[0, 0],  # Initial temperature
                    vol=0.0,  # Initial voltage
                    sensor=[0, 0],  # Initial sensor values
                    motorstate=0,  # Initial motor state
                    reserve=[0, 0, 0, 0]  # Reserved values
                ) for _ in range(7)  # 7 motors for left hand
            ]
            
            # Initialize right hand state
            self.right_hand_state = HandState_default()
            self.right_hand_state.motor_state = [
                MotorState_(
                    mode=0,  # Default mode
                    q=0.0,   # Initial position
                    dq=0.0,  # Initial velocity
                    ...
                    motorstate=0,  # Initial motor state
                    reserve=[0, 0, 0, 0]  # Reserved values
                ) for _ in range(7)  # 7 motors for right hand
            ]
            ...
            self.handStateThread = RecurrentThread(
                interval=1.0/config.HAND_CONTROL_FREQ,  # 100Hz
                target=self.PublishHandState,
                name="sim_handstate"
            )
        ...
        if self.mj_data != None:
            for i in range(self.body_motors):
                ...
                if i in self.wrist_motors:
                    gains = self.gains['wrist']
                elif i in self.weak_motors:
                    gains = self.gains['low_torque']
                else:
                    gains = self.gains['high_torque']

                # Apply control with motor-specific gains
                self.mj_data.ctrl[i] = (
                    msg.motor_cmd[i].tau
                    + gains['kp'] * (msg.motor_cmd[i].q - self.mj_data.sensordata[i])
                    + gains['kd'] * (msg.motor_cmd[i].dq - self.mj_data.sensordata[i + self.num_motor])
                )

    def LeftHandCmdHandler(self, msg: HandCmd_):
        if self.mj_data != None and self.hand_motors > 0:
            # Handle first 7 motors (left hand)
            for i in range(7):
                idx = self.body_motors + i
                self.mj_data.ctrl[idx] = (
                    msg.motor_cmd[i].tau
                    + msg.motor_cmd[i].kp
                    * (msg.motor_cmd[i].q - self.mj_data.sensordata[idx])
                    + msg.motor_cmd[i].kd
                    * (msg.motor_cmd[i].dq - self.mj_data.sensordata[idx + self.num_motor])
                )

    def RightHandCmdHandler(self, msg: HandCmd_):
        if self.mj_data != None and self.hand_motors > 0:
            # Handle second 7 motors (right hand)
            for i in range(7):
                idx = self.body_motors + 7 + i
                current_pos = self.mj_data.sensordata[idx]
                current_vel = self.mj_data.sensordata[idx + self.num_motor]
                cmd_pos = msg.motor_cmd[i].q
                cmd_vel = msg.motor_cmd[i].dq
                cmd_kp = msg.motor_cmd[i].kp
                cmd_kd = msg.motor_cmd[i].kd
                cmd_tau = msg.motor_cmd[i].tau
                
                # Safety checks
                if not np.isfinite(cmd_pos) or not np.isfinite(cmd_vel):
                    # Invalid command values for motor, skipping
                    continue
                
                if not np.isfinite(cmd_kp) or not np.isfinite(cmd_kd):
                    # Invalid gains for motor, skipping
                    continue
                
                # Clamp gains to reasonable values
                cmd_kp = np.clip(cmd_kp, 0.0, 50.0)
                cmd_kd = np.clip(cmd_kd, 0.0, 10.0)
                
                # Calculate control signal
                pos_error = cmd_pos - current_pos
                vel_error = cmd_vel - current_vel
                ...
                # Clamp final control signal
                control_signal = np.clip(control_signal, -10.0, 10.0)
                
                self.mj_data.ctrl[idx] = control_signal
    ...
    def PublishHandState(self):
        if self.mj_data != None and self.hand_motors > 0:
            ...
            for i in range(7):  # First 7 motors are left hand
                idx = self.body_motors + i
                self.left_hand_state.motor_state[i].q = self.mj_data.sensordata[idx]
                self.left_hand_state.motor_state[i].dq = self.mj_data.sensordata[idx + self.num_motor]
                self.left_hand_state.motor_state[i].tau_est = self.mj_data.sensordata[idx + 2 * self.num_motor]
            self.left_hand_state_puber.Write(self.left_hand_state)

            # Publish right hand state
            for i in range(7):  # Next 7 motors are right hand
                idx = self.body_motors + 7 + i
                self.right_hand_state.motor_state[i].q = self.mj_data.sensordata[idx]
                self.right_hand_state.motor_state[i].dq = self.mj_data.sensordata[idx + self.num_motor]
                self.right_hand_state.motor_state[i].tau_est = self.mj_data.sensordata[idx + 2 * self.num_motor]
