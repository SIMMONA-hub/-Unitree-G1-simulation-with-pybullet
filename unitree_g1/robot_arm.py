...
class G1_29_ArmController:
    ...
    def stop_all_motors(self):
        ...
        This will maintain current positions but stop any movement.
        """
        try:
            # Get current positions
            current_q = self.get_current_dual_arm_q()
            # Send zero velocity commands while maintaining current positions
            self.ctrl_dual_arm(current_q, np.zeros(14))
            # Send multiple times to ensure it's received
            for _ in range(5):
                self.ctrl_dual_arm(current_q, np.zeros(14))
                time.sleep(0.02)
        except Exception as e:
            print(f"Error in stop_all_motors: {e}")
